...
char* get_wifi_nvs_str(const char* value_name)
{
    ...
    if (required_size > 0) {
        err = nvs_get_str(nvs_handle, value_name, value, &required_size);
        if (err != ESP_OK) {
            free(value);
        }
    }
...
int set_wifi_nvs_str(const char* value_name, const char* value)
{
    nvs_handle_t nvs_handle;
    esp_err_t err;
    ...
    err = nvs_set_str(nvs_handle, value_name, value);
...
int store_wifi_password(const char* password)
{
...
int store_wifi_ssid(const char* password)
{
